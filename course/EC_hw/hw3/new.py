...
def fitnessFunc(x):
    #return 50.0 - x*x
     return -10.0 - ((0.04*x)**2) + 10.0 * np.cos(0.04*np.pi*x) 
...
def printStats(pop,gen):
    ...
    for p in pop:
        avgval+=p.fit
        mean = avgval/len(pop)
        avgdev_temp += abs(p.fit - mean)
        avgdev = avgdev_temp/len(pop)
        stddev_temp+=(p.fit - mean)**2
        stddev = np.sqrt(stddev_temp/len(pop))
        if p.fit > maxval: 
            maxval=p.fit 
            state_value = p.x            
        print(str(p.x)+'\t'+str(p.fit))
    plt.scatter(gen,maxval,color='b',label="best fitness")
    plt.scatter(gen,state_value,color='y',label='state value')
    plt.scatter(gen,avgdev,color='r',label="average deviation")
    plt.scatter(gen,stddev,color='g',label='standard deviation')
...
def main(argv=None):
    ...
    try:
        ...
        x_1 = np.arange(-100.0,100.0,1.0)
        y_1 = -10.0 - ((0.04*x_1)**2) + 10.0 * np.cos(0.04*np.pi*x_1)
        fig, ax1 = plt.subplots()
        ax1.plot(x_1, y_1)                
        ax1.grid()
        ax1.set(xlabel='x', ylabel='f(x)', title='fitness fuction')
        
        fig.savefig('ec_hw3.png')
        plt.show()
